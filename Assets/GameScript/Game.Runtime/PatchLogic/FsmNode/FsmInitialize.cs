...
internal class FsmInitialize : IStateNode
{
	...
	private IEnumerator InitPackage()
	{
		...
		if (playMode == EPlayMode.EditorSimulateMode)
		{
            ...
            //MotionEngine.CreateModule<ResourceManager>(createParameters);
        }
  ...
  //      }
	...
	private string GetHostServerURL()
	{
        ...
        string hostServerIP = "http://182.92.129.136";  /*"http://127.0.0.1";*/
		string appVersion = "v1.0";
        string projectName = "my_framework/";

#if UNITY_EDITOR
        if (UnityEditor.EditorUserBuildSettings.activeBuildTarget == UnityEditor.BuildTarget.Android)
            return $"{hostServerIP}/CDN/{projectName}Android/{appVersion}";
        else if (UnityEditor.EditorUserBuildSettings.activeBuildTarget == UnityEditor.BuildTarget.iOS)
            return $"{hostServerIP}/CDN/{projectName}IPhone/{appVersion}";
        else if (UnityEditor.EditorUserBuildSettings.activeBuildTarget == UnityEditor.BuildTarget.WebGL)
            return $"{hostServerIP}/CDN/{projectName}WebGL/{appVersion}";
        else
            return $"{hostServerIP}/CDN/{projectName}PC/{appVersion}";
#else
