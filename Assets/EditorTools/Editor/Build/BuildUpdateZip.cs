...
public class BuildUpdateZip : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("打热更包"))
        {
            ...
            ZipUpdateDir(zipDir, contionsObj, VersionMgr.instance.resVersion);
    ...
    private void ZipUpdateDir(string zipDir, bool contionsObj,string resVersion)
    {
        //var zipFilePath = BuildUtils.BIN_PATH + "/" + (contionsObj ? "res_" : "script_") + resVersion + ".zip";
        var zipFilePath = BuildUtils.BIN_PATH  +"res_"+ resVersion + ".zip";
    ...
    private void BuildLuaUpdateBundle(string zipDir)
    {
        ...
        Debuger.Log("needUpdateLuaFiles.Count:" + needUpdateLuaFiles.Count);
    ...
    private List<string> GetNeedUpdateLuaList()
    {
        ...
        if (File.Exists(originalLuaMD5FilePath))
        {
